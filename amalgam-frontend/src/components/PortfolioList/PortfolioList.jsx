...
function PortfolioList({ onPortfolioUpdate }){
    ...
    async function handleApiKeySave() {
        try {
            ...
            if (testResponse.data && testResponse.data.c) {
                ...
                await axios.post(`http://localhost:8085/api/users/${user.id}/finnhub-key`, {
                    finnhubKey: apiKey
                });
    ...
    useEffect(() => {
        async function loadApiKey() {
            try {
                const user = JSON.parse(localStorage.getItem('user'));
                const response = await axios.get(`http://localhost:8085/api/users/${user.id}/finnhub-key`);
    ...
    async function handleAddInvestment() {
        ...
        try {
            ...
            // Save to backend
            const response = await axios.post('http://localhost:8085/api/stocks', {
                userId: user.id,
    ...
    async function handleRemoveStock(index) {
        try {
            ...
            await axios.delete(`http://localhost:8085/api/stocks/${stockToRemove.id}`, {
                data: { 
    ...
    useEffect(() => {
        const fetchStocks = async () => {
            try {
                const user = JSON.parse(localStorage.getItem('user'));
                const response = await axios.get(`http://localhost:8085/api/stocks/user/${user.id}`);
